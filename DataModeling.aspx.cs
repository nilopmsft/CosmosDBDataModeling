...
{
    public partial class DataModeling : System.Web.UI.Page
    {
        ...
        protected void searchButton_Click(object sender, EventArgs e)
        {
            ...
            dmProcessQuery(searchValue, containerName, queryType, "none");
        ...
        protected void pointQueryId_ValueChanged(object sender, EventArgs e)
        {
            ...
            dmProcessQuery(searchValue, containerName, queryType, objectType);
        ...
        private async void dmProcessQuery(string searchValue, string containerName, string queryType, string objectType)
        {
            ...
            if (queryType == "sql")
            {
                ...
                queryData = await dmRunSqlQuery(sqlQueryText, containerName, singleQuerySelected);
            ...
            else
            {
                ...
                queryData = await dmRunPointRead(docId, movieTitle, containerName, objectType);
        ...
        private async Task<Dictionary<string, string>> dmRunSqlQuery(string sqlQueryText, string containerName, string singleQuerySelected)
        {

            Dictionary<string, string> queryData = new Dictionary<string, string>();

            dmCosmosConnection C = new dmCosmosConnection();
            C.connectionSetup();
            ...
            queryData.Add("searchResults", dmMovieSQLResults(movies, containerName, singleQuerySelected));

            return queryData;
        }


        private async Task<Dictionary<string, string>> dmRunPointRead(string docId, string objectTitle, string containerName, string objectType)
        {
            Dictionary<string, string> queryData = new Dictionary<string, string>();
            ...
            {
                queryData.Add("searchResults", dmActorPointResults(documentResponse.Resource, containerName));
            }
            else
            {
                queryData.Add("searchResults", dmMoviePointResults(documentResponse.Resource, containerName));
            }
            return queryData;

        }

        private static string dmMoviePointResults(dynamic movieDetails, string modelType)
        {
        ...
        private static string dmActorPointResults(dynamic movieDetails, string modelType)
        {
        ...
        public static string dmMovieSQLResults(List<dynamic> movies, string queryType, string singleQuerySelected)
        {
            string searchResults = "";
            List<string> knownTitles = new List<string>();
    ...
    public class dmCosmosConnection
    {
        // The Azure Cosmos DB endpoint for running this sample.
        private static readonly string EndpointUri = ConfigurationManager.AppSettings["dmEndPointUri"];

        // The primary key for the Azure Cosmos account.
        private static readonly string PrimaryKey = ConfigurationManager.AppSettings["dmPrimaryKey"];

        private string databaseId = ConfigurationManager.AppSettings["dmCosmosDB"];
